...
<!---2. Assign a Floating IP to the BIG-IP data subnet. Refer to the steps below to associate floating IP
#    - Go to `VPC Infrastructure Gen 2` from IBM Cloud
#    - Click `Floating IPs` from the left pane
#    - Click `Reserve floating IP`, enter `data-floating-ip` for its name -> Click `Reserve IP`
